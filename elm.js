...
Elm.DynamicStyle.make = function (_elm) {
   ...
   baseState,
   hoverState) {
      var jsName = function (cssAttr) {
         return function (s) {
            return A2($Basics._op["++"],
            $String.toLower(A2($String.left,1,s)),
         ...
         function (s) {
            return A2($Basics._op["++"],
            $String.toUpper(A2($String.left,1,s)),
            A2($String.dropLeft,1,s));
         },
         A2($String.split,"-",cssAttr))));
      };
      var toJS = function (attrs) {
         return A3($List.foldl,
         F2(function (_p0,x) {
            var _p1 = _p0;
            return A2($Basics._op["++"],
      ...
      _U.list([$Html$Attributes.style(A2($Basics._op["++"],
              ...
              function (_p4) {
                 var _p5 = _p4;
                 return {ctor: "_Tuple2",_0: _p5._0,_1: _p5._2};
              },
              hoverState)))]),
      A2($List.map,
      function (inactive) {
...
Elm.ReadText = Elm.ReadText || {};
Elm.ReadText.make = function (_elm) {
   "use strict";
   _elm.ReadText = _elm.ReadText || {};
   if (_elm.ReadText.values) return _elm.ReadText.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var listHtml = F2(function (address,display) {
      var writeSpan = function (token) {
         return A2($Html.div,
         _U.list([$Html$Attributes.$class("item")]),
         _U.list([$Html.text($Basics.toString(token.start))]));
      };
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("ui right sidebar vertical inverted menu")]),
      A2($List.map,writeSpan,display));
   });
   var textStyle = $DynamicStyle.hover(_U.list([{ctor: "_Tuple3"
                                                ,_0: "backgroundColor"
                                                ,_1: "white"
                                                ,_2: "#82caff"}]));
   var textUrl = "http://localhost:3000/en";
   var init = {ctor: "_Tuple2"
              ,_0: {text: "",tokens: _U.list([]),display: _U.list([])}
              ,_1: $Effects.none};
   var Model = F3(function (a,b,c) {
      return {text: a,tokens: b,display: c};
   });
   var Token = F2(function (a,b) {    return {start: a,end: b};});
   var decodeText = function () {
      var token = A3($Json$Decode.object2,
      Token,
      ...
      A2($Json$Decode._op[":="],"tokens",$Json$Decode.list(token)),
      A2($Json$Decode._op[":="],"display",$Json$Decode.list(token)));
   }();
   var NoOp = {ctor: "NoOp"};
   ...
   NewText,
   $Task.toMaybe(A2($Http.get,decodeText,textUrl))));
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2"
                           ,_0: model
                           ,_1: $Effects.none};
         case "ListWord": return {ctor: "_Tuple2"
                                 ,_0: _U.update(model,
                                 {display: A2($List._op["::"],_p0._0,model.display)})
                                 ,_1: $Effects.none};
         case "NewText": var _p1 = _p0._0;
           if (_p1.ctor === "Just") {
                 return {ctor: "_Tuple2",_0: _p1._0,_1: $Effects.none};
              } else {
                 return {ctor: "_Tuple2"
                        ,_0: {text: "nothing retrieved"
                             ,tokens: _U.list([])
                             ,display: _U.list([])}
                        ,_1: $Effects.none};
              }
         default: return {ctor: "_Tuple2",_0: model,_1: refreshFx};}
   });
   var Refresh = {ctor: "Refresh"};
   var ListWord = function (a) {
      return {ctor: "ListWord",_0: a};
   };
   var textHtml = F4(function (address,modelText,ind,tokens) {
      var _p2 = tokens;
      if (_p2.ctor === "::") {
            var _p3 = _p2._0;
            var betweenText = A3($String.slice,ind,_p3.start,modelText);
            var tokenText = A3($String.slice,
            _p3.start,
            _p3.end + 1,
            modelText);
            return A2($Basics._op["++"],
            _U.list([$Html.text(betweenText)
                    ,A2($Html.span,
                    A2($Basics._op["++"],
                    textStyle,
                    _U.list([A2($Html$Events.onClick,address,ListWord(_p3))])),
                    _U.list([$Html.text(tokenText)]))]),
            A4(textHtml,address,modelText,_p3.end + 1,_p2._1));
         } else {
            return _U.cmp(ind,
            $String.length(modelText)) < 0 ? _U.list([$Html.text(A3($String.slice,
            ind,
            $String.length(modelText),
            modelText))]) : _U.list([]);
         }
   });
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2(listHtml,address,model.display)
              ,A2($Html.div,
              _U.list([$Html$Attributes.$class("pusher")]),
              $List.concat(_U.list([A4(textHtml,
                                   address,
                                   model.text,
                                   0,
                                   model.tokens)
                                   ,_U.list([A2($Html.button,
                                   _U.list([A2($Html$Events.onClick,address,Refresh)]),
                                   _U.list([$Html.text("Refresh")]))])])))]));
   });
   return _elm.ReadText.values = {_op: _op
                                 ,ListWord: ListWord
                                 ,Refresh: Refresh
                                 ,NewText: NewText
                                 ,NoOp: NoOp
...
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $ReadText = Elm.ReadText.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StartApp = Elm.StartApp.make(_elm),
   $Task = Elm.Task.make(_elm);
   var _op = {};
   var app = $StartApp.start({init: $ReadText.init
                             ,update: $ReadText.update
                             ,view: $ReadText.view
                             ,inputs: _U.list([])});
   var main = app.html;
